...
function extractDebugTraces(batch) {
  if (Array.isArray(batch?.calls)) return batch.calls; // common Streams key
  if (Array.isArray(batch?.traces)) return batch.traces; // fallback
  if (Array.isArray(batch?.trace)) return batch.trace; // fallback
  return [];
}
...
function collectAddressesFromDebugTraces(tracesForTx, addrSet) {
  const stack = [];

  for (const item of Array.isArray(tracesForTx) ? tracesForTx : []) {
    // Accept both root-with-result and already-flattened nodes
    const root = item?.result ?? item;
  ...
  while (stack.length) {
    ...
    try {
      ...
      const children = n?.calls || n?.subtraces || n?.call_traces || [];
      if (Array.isArray(children)) {
        for (let i = 0; i < children.length; i++) stack.push(children[i]);
      }
    } catch (e) {
      console.error('debug_trace parse error:', e);
    }
  }
}
...
async function main(stream) {
  ...
  for (const batch of batches) {
    ...
    for (const tx of txs) {
      ...
      const tracesForTx = tracesByTx.get(h) || [];
      const tracesForTxNormalized = tracesForTx.map((it) => it?.result ?? it);

      const addrSet = new Set();
      collectAddressesFromTx(tx, addrSet);
      collectAddressesFromLogs(logs, addrSet);
      collectAddressesFromDebugTraces(tracesForTxNormalized, addrSet);
